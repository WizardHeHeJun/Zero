...
- **放数据跑真实训练**：**EmoBank 已实跑**（词袋版 loss 0.016；句向量升级版 loss 0.0056、跨域更稳，见阶段 10）；**RAVDESS 韵律已实跑**（Zenodo 免登录下载、全量 1440 条，loss 0.126→0.026，推理验证 pitch 随 arousal 单调上升的真实声学映射；speech_rate/energy 区分度弱，属 loader 特征代理的尺度问题）；其余 EULA-free 集（WESAD 生理）仍待放 `data/` 跑对应 `train_*`（脚手架已就绪）。
