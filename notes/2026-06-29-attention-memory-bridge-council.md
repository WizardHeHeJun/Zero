...
- 日期：2026-06-29
- 议题：补齐情感引擎⊗LLM数字人「短时注意力↔长时记忆」之间的桥——短时端 `history[-20:]` 死窗口、召回「拼字符串」未进注意力预算、召回线索单一无三维打分。三解法：A 有 token 预算的注意力装配器 / B `recall` 之上三维重排(recency×salience×relevance) / C 遗忘=时序失效+容量上限不物理删。
