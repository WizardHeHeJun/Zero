...
# τ（最小驻留）：**60ms 定档（2026-09-01 数值门·design §四「通过的最小 τ」）**——
# 真机三句网格（记录 PRP/lipsync-v2/t0-params-record 附录）：τ=60 的 M9 局部位移仅 3ms；
# τ=80 → 50ms、τ=100 → 181ms 均超 20ms 预算 ⇒ 三候选唯一过 M9 者即最小者。
# G4 观感终审留用户；调它先重跑数值门（M9 局部预算是硬约束，别按观感直接抬）。
TAU_MS = 60.0

# β（能量混合基底）：候选 [0.3, 0.5] 中点——M3 公式
# `factor = BETA + (1-BETA)*max(floor, env_norm)`；数值门检查动态范围不收缩
# （2026-09-01 实测 MouthOpen max≈0.85）✓，量值维持待 G4 观感终审。
BETA = 0.4

# 连续滑移率（ms^-1，非 v1 的 per-frame 定值）：**0.008 定档（2026-09-01 数值门）**——
# 0.002（v1 同速率）在音素时长尺度上到不了位：b/p/m 闭唇残留 max 0.71（嘴闭不上，
# 三类可辨直接失效）、元音达成率 min 0.17；0.008 → 残留 p50=0/max 0.29、达成率 p50=1.00，
# 且对应全闭合约 ~125ms，与真实构音速度同量级（不属「机械快」——v1 定慢是因能量包络
# 20fps 抖动，v2 目标是音素级稳态，两者约束对象不同，勿照搬 v1 速率直觉）。
# 非均匀关键帧下 `max_delta = SLEW_RATE * dt_ms`（M5），依赖渲染端线性插值语义
# （08-31 跨仓回执已终答，语义变更需重新论证）。G4 观感终审留用户。
SLEW_RATE = 0.008
