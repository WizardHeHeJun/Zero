"""本地命令行验证 map 情绪读出（ZERO_AFFECT_READOUT=map；无需 LLM / 无需服务，确定性）。

根因分析：`--chat` 连续敌意时「情绪标签逐轮翻号」（敌意却采到正 valence）的根因是
**先验量级弱**（软 appraise 给 goal_congruence≈-0.2，post_mu valence≈-0.1、贴 0 边界）
**+ 逐轮单样本采样的大方差**。两条机制互补：
  - `ZERO_APPRAISE_CALIBRATE`：把敌意标定拉到 -0.7~-0.9（先验远离 0 边界）。
  - `ZERO_AFFECT_READOUT=map`：e*=post_mu（MMSE 点估计，去掉采样方差）。

本脚本绕过 LLM、直接给引擎喂 goal_congruence<0 的 stimulus，统计每轮 e* 的 valence 符号，
对比 sample（默认）vs map 在三档先验强度下的翻号率——量化坐实两条机制各自的作用。

跑：python -m scripts.verify_affect_readout

期望（典型）：
  goal=-0.2（软 appraise 现场）：sample 翻号 ~20% / map 0%   → map 读出在弱先验下消翻号
  goal=-0.7（标定拉强后）      ：sample 0%      / map 0%   → 标定拉强先验也独立消翻号
...
async def main() -> None:
    ...
    print(f"{'goal_congruence':>16} | {'sample 翻号(默认)':>22} | {'map 翻号':>18}")
    print("-" * 64)
