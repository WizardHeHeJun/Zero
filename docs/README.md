...
- **`v2/`** — **现阶段（2026-06-30，含阶段 25–27：注意力↔记忆桥 · 情绪稳态回弹 · 人格注入；framework-current 图 2026-07-03 补阶段 33 follow-up 三实验流）权威架构图**。README 内嵌的就是这套。每张图有 `.mmd`（Mermaid 源，**当前 PNG 由它经飞书画板渲染**）+ `.json`（whiteboard-cli DSL，富样式备选）+ `.png`（渲染图）。
