...
| `ZERO_FACTUAL_MODE` | 关 | **事实化模式：是 AI 就是 AI，偏向事实陈述**。默认 prompt 要求模型「像真人」，模型便用人类图式补全自己没有的属性——100 轮实测里它报出具体日期「20号，周二」、编出姓名职业、描述「走到窗边撩开帘子」的身体动作，还把虚构行为当往事引用。`1` 开启后：摘掉「你是人/像真人」的身份断言，诚实条款从「记不清的别编」扩展到「无从知道的（日期/天气/自己的身世）直说没有这个信息」，且**只删身份、不删情绪**——引擎驱动的心情、脾气指令逐字保留，另加「不要说自己没有感情」的反塌陷条款。代价：system prompt 约 478→1989 字符（`--chat` 每轮多 ~1.5k 字符输入）。留空=关=旧行为逐字不变 |
