...
引擎与整条流水线的结构：评价桥 → 情感引擎（贝叶斯主动推断 + 显著度门控工作空间）→ 三时间尺度 → 双路语言 + 双通路表达 → 记忆/持久。

![情绪引擎框架图](docs/framework-current.png)

### 图二 · 项目运作流程：LLM ⊗ 情感引擎

**LLM 只在「输入」「输出」两端与情感引擎结合**（图中两个蓝框）：输入端把你的话**读成情绪**，输出端**被情绪调制着说话**；夹在中间产生情绪的是一套**确定性引擎**（红框，无 LLM），LLM 既不进情绪计算热路径、也不替仿生人"编造"记忆。
...
把"人产生并表达情绪"的过程，建模成一条**贝叶斯流水线**——感知一句话、推断出此刻的情绪、让它随时间演化、再分两路外化为语言和表情（上方「图一」即此结构）。
